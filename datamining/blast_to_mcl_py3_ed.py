...
def blast_to_mcl(blast_output,hit_frac_cutoff,exclude=EXCLUDE,\
				 ignore_interspecific_hits=IGNORE_INTRASPECIFIC_HITS,out_dir=None):

	#both percent query and hit coverage have to >= this
	
	print ("Reading raw blast output")
	print ("Taxa to exclude:" + "%s" %EXCLUDE)
	print ("Ignore intraspecific hit?"+ "%s" %IGNORE_INTRASPECIFIC_HITS)
	infile = open(blast_output,"r")
	if out_dir != None:
		basename = os.path.basename(blast_output)
		outname = out_dir + "/" + basename+".hit-frac"+str(hit_frac_cutoff)+".minusLogEvalue"
		outname1 = out_dir + "/" + basename+".ident"
	else:
		outname = blast_output+".hit-frac"+str(hit_frac_cutoff)+".minusLogEvalue"
		outname1 = blast_output+".ident"
	if len(EXCLUDE) > 0:
		outname += ".exclude"
	...
	print ("written to file %s.ident" %blast_output)
...
# 					print (line)
	...
	print ("Highly similar interspecific hits written to %s.ident" %blast_output)
...
if __name__ == "__main__":
	...
	blast_to_mcl(blast_output=sys.argv[1],hit_frac_cutoff=sys.argv[2])
